...
| #8 | Workspace ala NinjaAI | Chat app berubah jadi **workspace penuh** (bukan cuma tema): sidebar 6-navigasi (Home, KenOAI Assistant, My tasks + badge, Inbox + badge, Calendar, Reports & Analytics) + seksi "My projects" (3 projek seed: Product Launch ungu, Client Onboarding teal, Team Brainstorm orange) + seksi **Workspace files per-chat** + kartu Pro. Home: greeting "Hello, <nama>", ask bar → buat chat baru & kirim langsung, papan 3 kolom drag&drop (HIGH/NORMAL/LOW chips), widget Projects+Reminders, stat card +32% FAB, pill LIVE. View Tasks/Inbox/Calendar/Reports lengkap. File workspace (teks ≤512KB) di-inject ke system prompt sebagai `<<<FILE name>>>` — AI membaca file tiap chat. File baru: `src/Workspace.jsx`, `src/Workspace.css`; Sidebar.jsx & icons.jsx diperbarui; App.jsx + state view/projects/tasks/inbox/files + persist `kenoai_*`. File punya tombol preview + "Send to chat". Fitur mengikuti referensi NinjaAI (lihat Screenshot user). File korup = ditulis ulang via patch script python (`ws_app_patch.py`) |
